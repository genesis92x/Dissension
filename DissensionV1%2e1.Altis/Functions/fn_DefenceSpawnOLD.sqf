...
if (_HC) then
{
	...
	_grpGarrison setGroupOwner (owner HC);
	_grp setVariable ["DIS_TRANSFERED",true];
	_grpGarrison setVariable ["DIS_TRANSFERED",true];
};
...
[
	[_SSide,_SpwnedUnits,_TownReinforceBool,_SpawnAmount,_HalfPoint,_Pole,_StrongHoldBuildings,_grpGarrison,_infantrylist,_AirList,_AtkSide,_NameLocation,_MaxAtOnce,_PolePos,_DstReset,_CloseStill,_grp,_HC],
	{
		params ["_SSide","_SpwnedUnits","_TownReinforceBool","_SpawnAmount","_HalfPoint","_Pole","_StrongHoldBuildings","_grpGarrison","_infantrylist","_AirList","_AtkSide","_NameLocation","_MaxAtOnce","_PolePos","_DstReset","_CloseStill","_grp","_HC"];
		waitUntil
		{
			...
			if ((count _SpwnedUnits) < _MaxAtOnce) then
			{
				...
				private _SpwnArray = [_InfantryList,_grp,_Pole,_StrongHoldBuildings,_grpGarrison,_SSide,(_MaxAtOnce - (count _SpwnedUnits)),_ActiveBaddies] call DIS_fnc_DSpwnUnit;
...
waitUntil
{
	...
	{
		private _Grp = _x;
		if ((side _Grp) isEqualTo _SSide && {((leader _Grp) distance2D _PolePos < 400)}) then
		{
			{
				if (alive _x) then
				{
