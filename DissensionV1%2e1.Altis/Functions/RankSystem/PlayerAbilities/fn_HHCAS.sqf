...
if (DIS_PCASHNUM - 800 < 0) exitWith {systemChat "You lack sufficient funds!";if (playerSide isEqualTo west) then {playsound "EVAInsufficentFunds"} else {playsound "LEGIONInsufficentFunds"};};
if (DIS_AbilityCoolDown) exitWith {systemChat "Abilities still on cool-down.";};
DIS_PCASHNUM = DIS_PCASHNUM - 800;
playsound "Purchase";
...
private _VehC = "B_Heli_Attack_01_dynamicLoadout_F";
private _SpawnSide = playerSide;
private _Commander = DIS_WestCommander;
if (playerSide isEqualTo East) then {_VehC = "O_Heli_Attack_02_dynamicLoadout_F";_Commander = DIS_EastCommander;};
