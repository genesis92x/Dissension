...
if (DIS_PCASHNUM - 900 < 0) exitWith {systemChat "You lack sufficient funds!";if (playerSide isEqualTo west) then {playsound "EVAInsufficentFunds"} else {playsound "LEGIONInsufficentFunds"};};
if (DIS_AbilityCoolDown) exitWith {systemChat "Abilities still on cool-down.";};
DIS_PCASHNUM = DIS_PCASHNUM - 900;
playsound "Purchase";
DIS_AbilityCoolDown = true;[] spawn {sleep 120;DIS_AbilityCoolDown = false;};
["Transport Vehicle Drop In-bound",'#FFFFFF'] call Dis_MessageFramework;
private _VehC = "B_LSV_01_unarmed_F";
private _MkT = "b_motor_inf";
if (side (group player) isEqualTo East) then {_VehC = "O_MRAP_02_F";_MkT = "o_motor_inf";};
