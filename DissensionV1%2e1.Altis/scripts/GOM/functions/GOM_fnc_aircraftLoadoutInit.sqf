...
GOM_fnc_addAircraftLoadoutToObject = {
	...
	_action = ["Edit Aircraft Loadout",{params ["_caller"];[_caller] spawn GOM_fnc_aircraftLoadout},[],0,true,true,"","_this isEqualTo vehicle _this AND {speed _this < 3} AND {alive _this} AND {alive _target}"];
...
GOM_fnc_installPylons = {
	...
	if (DIS_PCASHNUM - 300 < 0) exitWith {systemChat "You lack sufficient funds!";};	
...
sleep random [0.5,1,2.5];
	...
	systemchat format ["Successfully installed %1 %2 on %3! -150$",_finalAmount,_magDispName,_pylonName];
	DIS_PCASHNUM = DIS_PCASHNUM - 300;	
	true
};
...
_mag = _activePylonMags select _forEachIndex;
	...
	systemchat "All pylons, counter measures and board guns rearmed! Charged 300$";
true
};
...
if (_abort) exitWith {true};
	...
	if (!_abort OR !_empty) then {systemchat format ["%1 repaired: %2%3!",_vehDispName,_damDisp,"%"];DIS_PCASHNUM = DIS_PCASHNUM - 300;} else {systemchat "Repair aborted!"};
		playSound "Click";
	};
...
if (_leaking AND fuel _veh > (1 - _leakingLevel)) exitWith {systemchat format ["This aircraft will leak fuel down to %1%2 unless repaired!",round ( (1 - _leakinglevel) * 100),"%"];systemchat "Stopping Refueling procedure due to fuel leak!";playsound "Simulation_Fatal";	_refuelSource setVariable ["GOM_fnc_aircraftLoadoutBusyFuelSource",false,true];
	...
	if (GOM_fnc_aircraftLoadout_NeedsFuelSource) then {
		...
		if (_timer % 10 isEqualTo 0) then {systemchat format ["%1 remaining: %2l, %3s.",_vehDispName,_missingFuel,_timeNeeded];
			...
			if (!_abort AND !_empty AND !_leaking) then {	systemchat format ["%1 filled up! -300%",_vehDispName];
				_refuelSource setVariable ["GOM_fnc_aircraftLoadoutBusyFuelSource",false,true];DIS_PCASHNUM = DIS_PCASHNUM - 300;
